...
`run_weather_generator()` is the entry point for a complete run. It generates
the synthetic series, reshapes them for evaluation, and evaluates them against
the observed record, returning both halves.

Settings are supplied as a single `config` list. Only `vars` and
`n_realizations` are required; anything you leave out falls back to that
function's own default, so a config names only what you want to change.
...
`run_weather_generator()` returns a list with two components:

- `gen_output`: what the generator produced
...
- `evaluation`: the diagnostic plots comparing simulated against observed,
...
stochastic_weather <- result$gen_output
...
The evaluation needs the synthetic series as daily values, not as the resampled
dates the generator returns. `run_weather_generator()` does that reshaping
internally, so the diagnostics are already in the result:

```{r}
#| label: run-evaluation
#| results: hide
evaluation_plots <- result$evaluation
```

If you call `generate_weather()` directly instead, you have to build that input
yourself before calling `evaluate_weather_generator()` -- map each realization's
resampled dates back onto the observed record, one grid cell at a time. The
section below on extracting synthetic weather shows the same indexing.
...
Each element of `evaluation_plots` is an ordinary ggplot object, so print one on
its own or the whole list at once:

```{r}
#| label: view-evaluation-usage
#| eval: false
# A single diagnostic
...
#| output: asis
#| echo: false
# The diagnostics do not all have the same shape: most are 2x2 panel grids, but
# the spell-length and wet/dry-day comparisons are a single row and the annual
# series is a single panel. Rendering every one at one fixed height stretches
# the short ones, so height follows the number of panel rows each plot actually
# draws while the width stays constant.
#
# One child chunk per plot because knitr does not recycle a vector fig-height
# across the plots printed by a single chunk -- it applies the last value to all
# of them.
panel_rows <- function(p) max(ggplot2::ggplot_build(p)$layout$layout$ROW)

# Caption and alt text come from the plot's own title and subtitle, so they stay
# in step with the figure rather than drifting as a separate hand-kept list.
#
# Both are emitted as single-line quoted YAML scalars, and several of these
# titles and subtitles carry embedded newlines for layout inside the plot, so
# they have to be flattened first or the scalar never closes.
yaml_scalar <- function(x) {
  x <- gsub('"', "'", x, fixed = TRUE)
  trimws(gsub("[[:space:]]+", " ", x))
}

for (nm in names(evaluation_plots)) {
  p <- evaluation_plots[[nm]]
  # Not `%||%`: that is base R only from 4.4.0 and this package supports 4.1.
  caption <- yaml_scalar(if (is.null(p$labels$title)) nm else p$labels$title)
  alt <- yaml_scalar(paste(c(caption, p$labels$subtitle), collapse = ". "))

  cat(
    knitr::knit_child(
      text = c(
        "```{r}",
        sprintf("#| label: fig-%s", gsub("_", "-", nm)),
        sprintf("#| fig-cap: \"%s\"", caption),
        sprintf("#| fig-alt: \"%s\"", alt),
        "#| fig-width: 10",
        sprintf("#| fig-height: %s", 3.5 * panel_rows(p) + 1),
        "#| echo: false",
        sprintf("evaluation_plots[[%s]]", dQuote(nm, q = FALSE)),
        "```"
      ),
      quiet = TRUE
    ),
...
#| eval: false
# Parallelism is a config entry like any other. Realizations are the unit of
# work, so there is nothing to gain from more cores than realizations.
result <- run_weather_generator(
  obs_data  = ncdata$data,
  obs_grid  = ncdata$grid,
  obs_dates = ncdata$date,
  out_dir   = output_dir,
  config    = modifyList(config, list(parallel = TRUE, n_cores = 4))
)
```
...
write_netcdf(
  ...
  grid = ncdata$grid,
  out_dir = output_dir,
  # origin_date is the date of the *first* time step, not a reference epoch:
  # the time coordinate is written relative to it. Passing `dates` as well has
  # write_netcdf() check that rather than take it on trust.
  origin_date = stochastic_weather$dates[1],
  dates = stochastic_weather$dates,
  calendar = "noleap",
