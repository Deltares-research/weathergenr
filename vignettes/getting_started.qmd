...
```{r view_evaluation_usage, eval = FALSE}
# A single diagnostic
...
```{r view_evaluation, output = "asis", echo = FALSE}
...
for (nm in names(evaluation_plots)) {
  cat(
    knitr::knit_child(
      text = c(
        sprintf(
          "```{r fig-%s, fig.width = 10, fig.height = %s, echo = FALSE}",
          nm, 3.5 * panel_rows(evaluation_plots[[nm]]) + 1
        ),
