...
climateSurface <- function(
  ...
  p <- ggplot(str.data, aes(x = .data[[variable.x]], y = .data[[variable.y]])) +
    ...
    coord_cartesian(
      xlim = range(variable.x.breaks),
      ylim = range(variable.y.breaks),
      expand = FALSE
    ) +
    # Set labs
    labs(
      x = variable.x.label, y = variable.y.label,
  ...
  if (!is.null(gcm.data)) {
    ...
    gcm_scenario_color <- c(
      "ssp126" = "#003466", "ssp245" = "#f69320",
    ...
    p <- p + geom_point(
      mapping = aes(
        x = .data[[variable.x]], y = .data[[variable.y]],
        color = scenario, shape = horizon
      ), data = gcm.data,
