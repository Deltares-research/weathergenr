...
simulateWeather <- function(
...

 {
  ...
  #Remove extra grids
  grid_select <- which(sapply(1:nrow(nc_data$grid_mat), function(x)
    !is.na(mean(nc_data$grid_mat$data[[x]]$temp_min))))

  climate_tidy <- nc_data$grid_mat[grid_select,] %>% mutate(id = 1:n())

  # Number of grids
  grids  <- climate_tidy$id
  ngrids <- length(grids)

  message(cat("\u2713", "|", "Historical data loaded (", ngrids, "grids)"))
  ...
  wavelet_comps <- waveletDecompose(variable = warm_annual,
        ...
        signif.level = ssig, plot = TRUE, out.path = out_path)
  message(cat("\u2713", "|", "Wavelet analysis & decomposition completed"))
  ...
  message(cat("\u2713", "|", rmax, "annual series generated by WARM"))
  ...
  message(cat("\u2713", "|", "Stochastic traces selected:", ncol(sim_annual_sub$subsetted),
    "out of", ncol(sim_annual_sub$sampled), "meeting selection criteria"))
