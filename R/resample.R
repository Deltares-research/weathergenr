...
#' logic is assumed.
#'
#' @param sim_annual_precip Numeric vector of length `n_years`. Synthetic
#'   annual precipitation totals generated by the annual model (e.g., WARM),
#'   indexed by simulated year.
#' @param obs_annual_precip Numeric vector. Observed annual precipitation totals
#'   corresponding to historical water years.
#' @param obs_daily_precip Numeric vector. Observed daily precipitation values
...
#' using annual KNN matching against `sim_annual_precip`. Daily weather is then
...
resample_weather_dates <- function(
  ...
  for (y in seq_len(n_years)) {
    ...
    obs_year_draw_idx <- knn_sample(
      candidates = obs_annual_precip,
      target = sim_annual_precip[y],
      k = k_annual,
      n = annual_knn_n,
      prob = TRUE,
      seed = if (is.null(annual_seed_by_year)) NULL else annual_seed_by_year[y]
    )
    ...
    if (!length(day0_candidates)) day0_candidates <- seq_along(obs_precip_sub)
    ...
    if (sim_daily_precip[year_day0_idx] <= wet_threshold_by_month_order[first_month_order_idx]) {
      sim_precip_state[year_day0_idx] <- 0L
    } else if (sim_daily_precip[year_day0_idx] <= extreme_threshold_by_month_order[first_month_order_idx]) {
      sim_precip_state[year_day0_idx] <- 1L
    } else {
      sim_precip_state[year_day0_idx] <- 2L
    }
    ...
    for (j in 2:365) {
      ...
      if (!length(obs_day_candidates)) {
        sim_daily_precip[t_sim] <- sim_daily_precip[t_prev]
      ...
      transition_match_pos <- match_transition_positions(
        state_from = sim_precip_state[t_prev],
        state_to = sim_precip_state[t_sim],
        precip_vec = obs_precip_sub,
      ...
      if (!length(transition_match_pos)) {
        obs_window_idx <- expand_indices(obs_day_candidates, offsets61, length(obs_precip_sub))
        transition_match_pos <- match_transition_positions(
          state_from = sim_precip_state[t_prev],
          state_to = sim_precip_state[t_sim],
          precip_vec = obs_precip_sub,
      ...
      obs_precip_day1 <- obs_precip_sub[obs_day1_idx]
      ...
      if (is.na(draw_idx) || draw_idx < 1L || draw_idx > length(obs_precip_day1)) {
        draw_idx <- sample(seq_along(obs_precip_day1), 1L)
      }

      sim_daily_precip[t_sim] <- obs_precip_day1[draw_idx]
...
#'
#' Returns a valid index from `candidate_precip` based on the provided `idx`.
