...
readNetcdf <- function(
  ...
  ncGetDimNames <- function(f, v) {
    return(unlist(lapply(f$var[[v]]$dim, function(x) {
      return(x$name)
    })))
  }
  ...
  return(list(
    ...
    date = datev,
    dimensions = nc_dim,
    attributes = c(nc_attribs, stats::setNames(
      list(nc_attribs_spref),
      spatial.ref
    ), global = list(nc_attribs_glob))
  ))
}
