...
#' @param evaluate.model.grid.num placeholder
...
simulateGriddedWeather <- function(
...

 {
  ...
  message(cat("\u2713", "|", warm.sample.size, "stochastic annual series generated"))
  ...
  if(evaluate.model) {
    ...
    if (!dir.exists(paste0(output.path, "evaluation/"))) {
              dir.create(paste0(output.path, "evaluation/"))}
    ...
    evaluateWegen(daily.sim = rlz_sample,
       ...
       output.path = paste0(output.path, "evaluation/"),
  ...
  for (n in 1:realization.num) {
    ...
    for (s in 1:smax) {
      ...
      writeNetcdf(
          ...
          file.suffix = paste0("_", n,"_", s)
      )
    }

  message(cat("\u2713", "|", "Scenarios saved for realization", n))
  }

  message(cat("\u2713", "|", "Results outputted to", s, "netcdf files at: ", output.path))
