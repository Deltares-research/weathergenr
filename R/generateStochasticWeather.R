...
generateStochasticWeather <- function(
  climate.data = NULL,
  climate.grid = NULL,
  year.start = NULL,
  month.start = 1,
...

 {
  ...
  resampled_dates <- lapply(1:realization.num,
  ...
  resampled_dates_df <- bind_cols(resampled_dates,
    .name_repair=~paste0("rlz_", 1:length(resampled_dates)))

  write.csv(resampled_dates_df, paste0(warm_path, "resampled_dates.csv"))
