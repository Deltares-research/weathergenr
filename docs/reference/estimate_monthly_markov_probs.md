...
- dirichlet_alpha:
  ...
  number of observed transitions in that month. Note this shrinks the
  prior itself with sample size, so its influence falls off faster than
  the usual fixed-alpha smoothing: with a month's worth of daily
  transitions from a few decades the effective alpha is a few
  hundredths.
  [`generate_weather`](https://deltares-research.github.io/weathergenr/reference/generate_weather.md)
  does not expose it and always uses the default.

- wet_q, extreme_q:

  Numeric in (0, 1). Quantiles defining the wet and extreme states. Used
  only as a fallback, to derive a pooled threshold for any month whose
  supplied `wet_threshold` or `extreme_threshold` is not finite.
  Defaults reproduce the previous fixed 0.2 / 0.8 behaviour; pass the
  values the rest of the run uses so a degenerate month does not
