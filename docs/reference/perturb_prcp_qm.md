...
- month:
  ...
  Integer vector (same length as \`prcp\`). Calendar month for each day
  (1-12).

- year:

  Integer vector (same length as \`prcp\`). Simulation year index for
...
- validate_output:
  ...
  Logical. If TRUE, checks output for NaN/Inf and replaces

- diagnostics:

  Logical. If TRUE, return diagnostics information in the output.

- seed:

  Integer. Optional random seed used when diagnostics require stochastic
  elements. with original values. Default is TRUE.
...
Numeric vector, same length as \`prcp\`. Precipitation time series
...
\`min_events\` non-zero days are skipped. The gamma distribution is
...
parameters by scaling base mean and variance 3. Map each original prcp
