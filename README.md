...
1.  Low-frequency variability (WARM): Interannual to decadal climate
    ...
    components of the observed signal, which are stochastically
    simulated using autoregressive processes and recombined to generate
    synthetic annual sequences that preserve observed persistence and
    spectral structure. These annual simulations condition the daily
...
2.  Daily weather generation (Markov chain + KNN): Daily precipitation
    ...
    k-nearest-neighbour (KNN) resampling. The KNN approach preserves
    cross-variable dependence, seasonality, and spatial correlations
    across grid cells. Historical years are selected conditionally based
    on similarity to the simulated annual climate state, linking daily
...
3.  Climate perturbations and quantile mapping: Simulated weather series
    ...
    variability, or extremes-while retaining the internally consistent
    temporal and spatial structure generated by the stochastic model.
