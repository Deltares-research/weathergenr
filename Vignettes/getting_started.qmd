...
format: 
  html:
    ...
    toc-depth: 3  # Optional: controls heading levels shown
execute:
  echo: true
  warning: false
  eval: true
...
First lets load the packages required
...
We use generate_weather_series() to simulate new weather sequences from historical record using a coupled a Wavelet Autoregressive Model (WARM) with a a Markov Chain and KNN resampling scheme (See: [Steinscheineder et al (2013)](https://agupubs.onlinelibrary.wiley.com/doi/full/10.1002/wrcr.20528)

Define output path, variables, and simulation parameters. **realization_num** parameters sets how many stochastic replicates will be generated.
...
stochastic_weather <- generate_weather_series(
  ...
  warm.subset.criteria = list(mean = NULL, sd = NULL, min = NULL, max = NULL,
      sig.thr = NULL, nsig.thr = NULL),
  knn.sample.num = 100,
  mc.wet.quantile = 0.3,
...
```{r eval_plots1, eval=TRUE, echo=TRUE}
out
```
